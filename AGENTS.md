...
## Project overlay: nano-dlna mobile rewrite
- The existing product source of truth remains `web/backend` plus `web/frontend`; new cross-platform rewrite work belongs in `mobile-app/`.
- Treat the FastAPI backend as shared infrastructure. Reuse existing route groups such as `/api/devices`, `/api/videos`, `/api/streaming`, `/api/renderer`, `/api/overlay`, `/api/mappings`, `/api/projection`, and `/api/v2/discovery` before proposing backend duplication.
- For mobile tasks, preserve the current web dashboard unless the user explicitly asks to change it.
- Default mobile team shape: `architect` maps current features and API boundaries, `executor` implements in `mobile-app/`, and `verifier` checks Expo type safety plus any touched backend API behavior.
- Default team objective for this repo: recreate an operator-first iOS/Android client for device discovery, media browsing, streaming diagnostics, and staged access to renderer/overlay/projection capabilities.
- Preferred kickoff example: `omx team 3:executor "Continue the nano-dlna mobile rewrite in mobile-app using the existing FastAPI endpoints as the control plane."`
